...
$$
$$
{\Sigma_n x_n\over N}=3.46329
$$
It must not be the correct mean. 

The likelihood is as followings by the description of the problem. 
$$
P(\{x_n\}|\sigma_n,\mu) = ({1 \over 2\pi })^{N/2} \Pi_n{1\over \sigma_n } \exp(-\Sigma_n {(x_n-\mu)^2 \over 2\sigma^2_n})
$$ 
To find the maximum likelihood,
$$
L \equiv \log P(\{x_n\}|\sigma_n,\mu) = -\Sigma_n \log~\sigma_n-\Sigma_n {(x_n -\mu)^2 \over 2\sigma_n^2} =0 
$$
$$
{\partial L \over \partial x_n} = -\Sigma_n \left({x_n-\mu \over \sigma^2_n}\right)=-\Sigma_n{x_n \over \sigma_n^2}-\mu~\Sigma_n {1 \over \sigma_n^2}
$$
$$
\mu= {\Sigma_n{x_n\over \sigma_n^2}\over{1\over \sigma_n^2}}
$$
...
Given
$$
P(\{x_n\}|\sigma_n,\mu) = ({1 \over 2\pi })^{N/2} \left( \Pi_n{1\over \sigma_n }\right) \exp\left(-\Sigma_n {(x_n-\mu)^2 \over 2\sigma^2_n}\right)
$$ 
and
$$
P(\sigma_n|\mu)=({1\over \Gamma(c)~s^c})^N\Pi_n~(\sigma_n)^{c-1}~\exp({\Sigma_n \sigma_n \over s})\footnote{not sure if it is right.}
$$
where $(s,~c)=(10,0.1)$.
